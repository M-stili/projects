...
const spanCitation = document.querySelector('.citation');
const spanYear = document.querySelector('.year');
const body = document.querySelector('body');

function changeQuote() {

    let q_length = quotes.length;
    let randomQuote= Math.floor(Math.random()* q_length);
    paragraph.innerHTML = `"${quotes[randomQuote].quote}"`;
    author.innerHTML = `-${quotes[randomQuote].source}`;
	if (quotes[randomQuote].citation) {
		spanCitation.innerHTML = `,${quotes[randomQuote].citation}`;	
	}else{
		spanCitation.innerHTML = '';	
	};
	if (quotes[randomQuote].year) {
		spanYear.innerHTML = `,${quotes[randomQuote].year}`;
	}else{
		spanYear.innerHTML = '';	
	};
